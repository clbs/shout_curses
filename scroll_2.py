...
class Screen(object):
    ...
    def create_panel(self, panel_title, height, width, x_coordinate, y_coordinate):
    ...
    def display(self):
        ...
        self.window.erase()
        self.create_panel("results", 22, 40, 1, 20)
        curses.panel.update_panels()  
        for idx, item in enumerate(self.items[self.top:self.top + self.max_lines]):
            # Highlight the current cursor line
            if idx == self.current:
                self.window.addstr(idx + 10, 1, item, curses.color_pair(2))
            else:
                self.window.addstr(idx + 10, 1, item, curses.color_pair(1))
        self.window.refresh()
...
def main():
    ...
    screen = Screen(search_item_titles)
    screen.run()
